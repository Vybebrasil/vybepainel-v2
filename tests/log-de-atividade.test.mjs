...
async function banco({ semAutomacoes = false } = {}) {
  ...
  await db.exec(`
    CREATE TABLE vybe_pessoas (id int primary key, nome text, monday_user_id text, foto_url text);
    ...
    INSERT INTO vybe_pessoas VALUES (1, 'Deivid', '68997024', 'https://foto.example/deivid.jpg'), (2, 'Jady', '70000001', NULL);
...
test('o log junta eventos, automações e comentários do Monday, do mais recente ao mais antigo', async () => {
  ...
  // A foto e a referência da pessoa vão junto, para a tela desenhar a bolinha.
  assert.equal(itens[2].autor_ref, '68997024');
  assert.equal(itens[2].autor_foto, 'https://foto.example/deivid.jpg');
  assert.equal(itens[3].autor_foto, undefined);
...
test('cada tipo de evento vira uma frase, com o filtro certo', () => {
  const f = tela();
  // Sem o catálogo de status na sandbox, o status sai em negrito; na tela, é a etiqueta colorida.
...
test('status vira a etiqueta do catálogo, e quem fez vira bolinha com nome no title', () => {
  const c = vm.createContext({ console, document: { addEventListener() {} },
    safeText: (v) => String(v ?? '').replace(/[&<>"']/g, (ch) => `&#${ch.charCodeAt(0)};`),
    pillHtml: (s) => `<span class="pill">${s}</span>`,
    TEAM_USERS: [{ id: '68997024', name: 'Deivid', color: '#fdab3d', photo: 'https://foto.example/d.jpg' }] });
  vm.runInContext(fs.readFileSync('vybe-log.js', 'utf8'), c);
  c.ev = { tipo: 'status', de: 'Pode Fazer', para: '<i>x</i>', autor: 'Deivid', autor_ref: '68997024' };
  vm.runInContext('frase = fraseDoLog(ev).frase; rosto = rostoDoLogHtml(ev); robo = rostoDoLogHtml({ tipo: "automacao" });', c);
  assert.equal(c.frase, 'mudou o status de <span class="pill">Pode Fazer</span> para <span class="pill">&#60;i&#62;x&#60;/i&#62;</span>');
  assert.match(c.rosto, /<img src="https:\/\/foto.example\/d.jpg"/);
  assert.match(c.rosto, /title="Deivid"/);
  assert.match(c.robo, /title="Automação"/);
});
