...
function focusDailyPlanHtml(items,user,nextAction) {
  ...
  return `<section class="focus-daily-plan"><div class="focus-daily-plan-head"><div><span>Plano do dia</span></div><button type="button" class="focus-command-btn" onclick="abrirNotas()" title="Suas anotações do dia e sobre as demandas">Notas</button><button type="button" class="focus-command-btn" onclick="openFocusShiftClose()">Fechar turno</button></div><div class="focus-daily-plan-grid">${row('AGORA',inProgress[0],'nada em execução agora')}${''/* "PRÓXIMA" saiu daqui: o bloco grande logo abaixo E a proxima, com o mesmo
