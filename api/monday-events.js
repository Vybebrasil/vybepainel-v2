...
import { sincronizarDoEvento } from '../vybe_dominio_store.js';
...
export default async function handler(req, res) { cors(res); if (req.method === 'OPTIONS') return res.status(200).end(); if (req.method !== 'POST') return res.status(405).json({ error: 'Método não permitido.' }); const body = req.body || {}; if (body.challenge) { return res.status(200).json({ challenge: body.challenge }); } if (!webhookAutorizado(req)) return res.status(401).json({ error: 'Evento não autorizado.' }); const event = body.event || body; try { const registration = await registerEvent(event); if (!registration.accepted) return res.status(200).json({ ok: true, duplicate: true, trigger_uuid: registration.trigger_uuid }); try { const mirrored = await mirrorItemFromEvent(event); await markEventProcessed(registration.trigger_uuid);
    ...
    // endpoint existir.
    const bd = neon(process.env.DATABASE_URL);
    // Primeiro o dominio recebe a mudanca, so depois as regras rodam em cima
    // dela — inverter faria a regra decidir olhando o estado velho.
    try { await sincronizarDoEvento(bd, event); }
    catch (erro) { console.error('Sincronizacao do dominio pelo webhook falhou:', erro.message); }
    try { await aplicarDeEvento(bd, event); }
