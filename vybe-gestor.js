...
function vybeDono(item, className = '', opcoes = {}) {
  return ownerEditorTrigger(item, className, opcoes);
}
...
// comNome: no cartão de resumo a foto sozinha não dizia quem é — as outras linhas
// mostram texto. O primeiro nome vai ao lado, dentro do mesmo botão.
function ownerEditorTrigger(item, className='', { comNome = false } = {}) { const owners=ownerUsersFor(item); const title=owners.length?`Gerenciar responsáveis: ${owners.map(user=>user.name).join(', ')}`:'Adicionar responsável'; const visual=owners.length?owners.slice(0,3).map(ownerAvatarHtml).join('')+(owners.length>3?`<span class="owner-avatar-fallback" style="background:#465363">+${owners.length-3}</span>`:''):`<span class="owner-avatar-add">+</span>`; return `<button type="button" class="owner-editor-trigger ${className}" onclick="openOwnerEditor(event,'${item.id}')" title="${safeText(title)}" aria-label="${safeText(title)}"><span class="owner-avatar-stack">${visual}</span>${comNome ? `<span class="owner-nomes">${safeText(owners.length ? owners.map(user=>firstName(user.name)).join(', ') : 'Adicionar')}</span>` : ''}</button>`; }
