...
function managerCalendarItems() {
  ...
  return [...production, ...requests].filter(item => {
    ...
    if (managerCalendarClientFilter !== 'all' && item.cliente !== managerCalendarClientFilter) return false;
...
function managerCalendarEventHtml(item) {
  ...
  return `<button type="button" draggable="true" class="manager-calendar-event" style="--event-color:${color}" title="${safeText(`${item.nome} · ${item.cliente} · ${status} · arraste para mover a data`)}" onclick="managerCalendarOpen('${item.calendarSource}','${item.id}')" ondragstart="managerCalendarDragStart('${item.calendarSource}','${item.id}',event)" ondragend="managerCalendarDragEnd()"><span class="manager-calendar-event-bar"></span><span class="manager-calendar-event-copy"><b>${safeText(item.nome || 'Sem título')}</b><small>${safeText(item.cliente || '—')} · ${safeText(owner)} · <span style="color:${color}">${safeText(status)}</span></small></span><span class="manager-calendar-event-meta"><i class="${sourceClass}"></i><em class="manager-calendar-event-age">${sourceLabel}</em></span></button>`;
}
...
function renderManagerCalendar() {
  ...
  const allItems = managerCalendarItems();
  const monthItems = allItems.filter(item => meta.cells.some(cell => cell.iso === item.calendarDateIso));
  const clients = managerCalendarClientList(allItems, meta);
