...
function commandResponsibleHtml(d) {
  ...
  // Só as bolinhas: o nome fica no title, como no log de atividade. A linha da
...
function leituraProativaHtml(items=DADOS) {
  ...
  return `<section class="leitura-proativa"><div class="leitura-proativa-topo"><strong>Leitura proativa</strong><span>Atualiza a cada leitura do painel</span></div>${cards}</section>`;
}
...
function renderManagerIntelligence() {
  ...
  const tabs=[{id:'critical',label:'Resolver hoje',count:critical.length},{id:'blockers',label:'Bloqueios',count:blockers.length},{id:'external',label:'Depende do cliente',count:allExternal.length},{id:'internal',label:'Ajuste interno',count:internal.length},{id:'capacity',label:'Capacidade',count:0}];
  ...
  // A Mesa de Comando e fixa e tem z-index 205, mas mora dentro de uma secao com
  // isolation:isolate — e isso prende o empilhamento dela ali dentro. O 205 so
  // vale entre irmaos da mesma secao; qualquer coisa desenhada depois no
  // documento, como a tabela por grupos, passava por cima da Mesa inteira. Com a
  // Mesa aberta, a secao inteira sobe; fechada, tudo volta ao lugar.
  wrap.classList.toggle('com-mesa-de-comando', Boolean(managerCommandDrawerOpen));
  wrap.innerHTML = `<div class="manager-command-backdrop ${managerCommandDrawerOpen?'open':''}" onclick="closeManagerCommandDrawer()"></div><section class="manager-command-drawer ${managerCommandDrawerOpen?'open':''}" role="dialog" aria-modal="true" aria-label="Mesa de Comando" onclick="event.stopPropagation()"><header class="manager-command-drawer-head"><div><div class="manager-command-drawer-title">Mesa de Comando</div><div class="manager-command-drawer-context">${safeText(managerCommandContextLabel())}</div></div><button type="button" class="manager-command-close" onclick="closeManagerCommandDrawer()" aria-label="Fechar Mesa de Comando">×</button></header><div class="manager-command-summary">${priorityCard}</div><nav class="manager-command-tabs" aria-label="Seções da Mesa de Comando">${tabs.map(tab=>`<button type="button" class="manager-command-tab ${activeTab===tab.id?'active':''}" aria-pressed="${activeTab===tab.id}" onclick="setManagerCommandTab('${tab.id}')">${tab.label}${tab.count?`<b>${tab.count}</b>`:''}</button>`).join('')}</nav><div class="manager-command-content">${leituraProativaHtml(scopedItems)}${activeContent}</div></section>`;
...
// QUEM DECIDE SE O CLIENTE APARECE E O MESMO QUE DECIDE QUEM DESATIVAR.
//
// Eu tinha montado aqui um segundo conjunto de nomes ativos, com a minha propria
// comparacao. O resultado foi previsivel: desativar a ACE funcionava — porque a
// gravacao usa cadastroDoCliente, que compara nome exato E aproximado — e a
// lista continuava mostrando, porque a minha comparacao so olhava igualdade
// exata de duas formas do nome. Duas reguas para a mesma pergunta sempre
// divergem; a unica saida e nao ter a segunda.
//
// Agora a lista pergunta ao MESMO cadastro, pela MESMA funcao. "Desativei" e
// "sumiu" passam a ser o mesmo fato, e nao dois que precisam concordar.
let CADASTRO_PEDIDO_PELA_LISTA = null;
function garantirClientesAtivos() {
  if (typeof ensureClientMasterSources !== 'function') return null;
  if (CADASTRO_PEDIDO_PELA_LISTA) return CADASTRO_PEDIDO_PELA_LISTA;
  CADASTRO_PEDIDO_PELA_LISTA = Promise.resolve(ensureClientMasterSources())
    // Chegou depois da tela desenhada: redesenha para os inativos sairem.
    .then(() => { if (typeof renderManagerCalendar === 'function') renderManagerCalendar(); })
    .catch(() => { /* sem cadastro, a lista fica como estava */ });
  return CADASTRO_PEDIDO_PELA_LISTA;
}
...
function clienteApareceNaLista(nome) {
  ...
  // Cadastro ainda nao chegou: esconder por engano e pior do que mostrar a mais.
  if (typeof cadastroDoCliente !== 'function'
    || typeof CADASTRO_CLIENTES === 'undefined' || !CADASTRO_CLIENTES.length) return true;
  // A busca aproximada exige seis letras dos dois lados, entao um nome curto como
  // "ACE" nao alcanca "ACE - Associacao Comercial de Irece (ACE)". O apelido do
  // vybe-config resolve justamente isso — e continua sendo a MESMA busca, so com
  // o nome por extenso.
  const ficha = cadastroDoCliente(nome)
    || (typeof normalizarCliente === 'function' ? cadastroDoCliente(normalizarCliente(nome)) : null);
  // O cadastro nao conhece este nome: ele tem conteudo na tela, e sumir calado e
  // o pior comportamento possivel — ninguem tem como saber que existia.
  if (!ficha) return true;
  return ficha.ativo !== false;
}
...
async function tirarClienteDoPainel(nome, evento) {
  ...
  // gravarCadastroDeCliente ja releu o cadastro; falta a tela saber disso.
...
// e o resto fica atras de um "+N" que abre a mesma lista com busca.
function modoDaListaDeClientes() {
  try {
    const guardado = localStorage.getItem('vybe_calendario_clientes');
    return ['busca', 'fichas', 'resumo'].includes(guardado) ? guardado : 'resumo';
  } catch { return 'resumo'; }
}
function trocarModoDaListaDeClientes() {
  // Gira entre as tres formas, na ordem em que se precisa delas: o resumo do
  // dia a dia, a lista inteira quando se quer varrer, e a busca quando se sabe
  // o nome.
  const proximo = { resumo:'fichas', fichas:'busca', busca:'resumo' }[modoDaListaDeClientes()];
  try { localStorage.setItem('vybe_calendario_clientes', proximo); } catch { /* navegador sem armazenamento */ }
  renderManagerCalendar();
}
function verTodosOsClientes(sim) {
  try { localStorage.setItem('vybe_calendario_clientes', sim ? 'fichas' : 'resumo'); }
...
function renderManagerCalendar(forcar = false) {
  ...
  const alternar = `<button type="button" class="manager-calendar-modo" onclick="trocarModoDaListaDeClientes()"
      title="${modoClientes==='resumo' ? 'Ver todos os clientes lado a lado'
        : modoClientes==='fichas' ? 'Trocar por uma busca por nome'
        : 'Voltar para os principais do mês'}"
      aria-label="Trocar a forma de escolher o cliente">${
        modoClientes==='resumo' ? '▤' : modoClientes==='fichas' ? '⌕' : '⋯'}</button>`;
  ...
  const escolhidoDeFora = managerCalendarClientFilter !== 'all'
    && !primeiros.some((c) => c.client === managerCalendarClientFilter)
    ? clients.find((c) => c.client === managerCalendarClientFilter) : null;
  const mostrados = [...primeiros, ...(escolhidoDeFora ? [escolhidoDeFora] : [])];
  const restantes = clients.length - mostrados.length;
  const resumo = [
    `<button type="button" class="manager-calendar-client ${managerCalendarClientFilter === 'all' ? 'active' : ''}"
      onclick="managerCalendarSetClient('all')"><b>Todos</b> ${totalNoMes}</button>`,
    ...mostrados.map(ficha),
    restantes > 0 ? `<button type="button" class="manager-calendar-client escolha" onclick="abrirBuscaDeCliente(event)"
      title="Ver os outros ${restantes} clientes"><b>+${restantes}</b><i class="cliente-seta">▾</i></button>` : '',
    `<button type="button" class="manager-calendar-vertodos" onclick="verTodosOsClientes(true)">ver todos</button>`,
  ].filter(Boolean).join('');
  const fichasComRecolher = fichas
