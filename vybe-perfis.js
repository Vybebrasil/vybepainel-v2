...
function daPlanningDonosHtml(item) {
  ...
  if (!donos.length) return '<span class="owner-avatar-add" title="Sem responsável">+</span>';
  const naMesa = new Set(daPlanningPessoasDaMesa().map(String));
  const nomes = donos.map((u) => u.name).join(', ');
  return `<span class="owner-avatar-stack" title="${safeText(nomes)}">${donos.slice(0, 4).map((u) => {
    const fora = naMesa.size && !naMesa.has(String(u.id)) ? ' fora-da-mesa' : '';
    return `<span class="da-planning-dono${fora}">${ownerAvatarHtml(u)}</span>`;
  }).join('')}${donos.length > 4 ? `<span class="owner-avatar-fallback" style="background:#465363">+${donos.length - 4}</span>` : ''}</span>`;
}
