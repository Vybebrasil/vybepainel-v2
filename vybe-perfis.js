...
function daTacticalFormatTag(item, mostrarOrigem=true) {
  return `<span class="da-content-tags">${fmtHtml(daTacticalFormat(item))}${mostrarOrigem?operationalOriginTag(item):''}</span>`;
}
...
function daTacticalStatusTag(item, withAge=false, {semRiscoDaData=false}={}) { const risk=semRiscoDaData?null:daControllerRisk(item); const escalation=daCriticalEscalation(item); const riskHtml=escalation.level?`<span class="da-risk-mini ${escalation.level==='extreme'?'extreme':'critical'}">⚑ ${escalation.label}</span>`:risk?`<span class="da-risk-mini ${risk.level}">${risk.level==='critical'?'⚑':risk.level==='high'?'!':'◌'} ${risk.label}</span>`:''; const apoio=[riskHtml, withAge?daStatusAgeTag(item):''].filter(Boolean);
