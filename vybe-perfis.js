...
function renderDaControllerTactical() {
  ...
  </section>`; const directionStatuses=['Alteração','Falta Info','Falta D.A','Para aprovação','Ag. Interno','Aguardo','Ag. Aprovação Cliente','Ag. Info Cliente']; const withoutPrimary=item=>String(item.id)!==String(priority?.id||''); const backlog=backlogCritical.filter(withoutPrimary).slice(0,5); const backlogCount=backlog.length;
