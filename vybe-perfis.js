...
function estadoDasEstacoes() {
  ...
  return {
    gestor:    { n: risco, rot: risco === 1 ? 'atividade da equipe em risco' : 'atividades da equipe em risco', alerta: risco > 0 },
    foco:      minha
      ? { n: minha.abertas,
          ...
          rot: minha.atrasadas
            ? `na sua fila · ${minha.atrasadas} atrasada${minha.atrasadas === 1 ? '' : 's'}`
            : 'na sua fila',
    ...
    controler: { n: emAtencao, rot: `de ${comFila} ${comFila === 1 ? "pessoa" : "pessoas"} com fila em atenção`, alerta: emAtencao > 0 },
    ...
    clientes:  { n: clientes, rot: clientes === 1 ? 'cliente cadastrado' : 'clientes cadastrados', alerta: false },
  };
}
...
const DA_DISCIPLINES={audiovisual:{key:'audiovisual',label:'Audiovisual',short:'Vídeo e motion',color:'#ff8b2b',members:EQUIPES.audiovisual},design:{key:'design',label:'Design',short:'Cards, fotos e carrosséis',color:'#ff5f8f',members:EQUIPES.design}};
...
// "CONTEÚDO" repetido em nove linhas seguidas nao informa nada — informa a
// EXCECAO. Quando a lista inteira e da mesma origem, a etiqueta some; quando ha
// uma solicitacao no meio de conteudos, ela aparece e salta.
function daTacticalFormatTag(item, mostrarOrigem=true) {
  // Conteúdo é o normal da célula: numa fila mista, só a solicitação precisa de marca.
  return `<span class="da-content-tags">${fmtHtml(daTacticalFormat(item))}${mostrarOrigem&&isRequestItem(item)?operationalOriginTag(item):''}</span>`;
}
// semRiscoDaData: na fila de um dia, "Hoje", "Amanhã" ou "Atrasado" é igual em
// todas as linhas e já está no título da fila — repetir em cada peça é ruído.
// O alerta de etapa parada (escalation) continua: esse muda de peça para peça.
// soAtraso: numa fila "de hoje", "Hoje" é igual em todas as linhas, mas
// "Atrasado" (o prazo já passou) continua sendo notícia sobre aquela peça.
function daTacticalStatusTag(item, withAge=false, {semRiscoDaData=false, soAtraso=false}={}) { const riscoDaData=semRiscoDaData?null:daControllerRisk(item); const risk=soAtraso&&riscoDaData?.level!=='critical'?null:riscoDaData; const escalation=daCriticalEscalation(item); const riskHtml=escalation.level?`<span class="da-risk-mini ${escalation.level==='extreme'?'extreme':'critical'}">⚑ ${escalation.label}</span>`:risk?`<span class="da-risk-mini ${risk.level}">${risk.level==='critical'?'⚑':risk.level==='high'?'!':'◌'} ${risk.label}</span>`:''; const apoio=[riskHtml, withAge?daStatusAgeTag(item):''].filter(Boolean);
...
function daQuickWeekdaysHtml(range,items) { if(daControllerPeriod!=='week') return ''; const labels=['SEG','TER','QUA','QUI','SEX']; return `<div class="da-quick-weekdays"><span class="da-quick-label">Foco rápido</span>${labels.map((label,offset)=>{const iso=daControllerIsoAt(range.start,offset); const count=items.filter(item=>daControllerDate(item)===iso && !['Para agendar','Agendado'].includes(operationalFlowStatus(item))).length; const active=daControllerDayFocusIso===iso; return `<button type="button" class="da-quick-day ${active?'active':''}" onclick="setDaQuickWeekday('${iso}')" title="${active?'Limpar':'Focar'} ${label} ${daAgendaDayInfo(iso).date}">${label}<span>${count}</span></button>`;}).join('')}</div>`; }
// A data da linha é o próprio botão que a edita, como na ficha: "Prazo 17/09"
// abre a caixa das duas datas. Antes era um relógio ◷ ao lado do texto em caixa
// alta — dois elementos para uma coisa, e o que se clicava não dizia o que era.
function daBotaoDeDataHtml(item, rotulo) {
  const gap=goldenDeadlineGap(item?.prazo_iso,item?.veiculacao_iso); const risco=gap!==null&&gap<PRAZO_OURO_DIAS;
  const titulo=risco
    ? `Menos de ${PRAZO_OURO_DIAS} dias entre prazo e veiculação (Prazo de Ouro): ${gap} ${gap===1?'dia':'dias'}. Clique para editar as datas.`
    : `Editar prazo e veiculação${gap===null?'':` · ${gap} dias entre as duas`}`;
  return `<button type="button" class="da-data-botao ${risco?'gold-risk':''}" onclick="openPlanningEditor('${item.id}')" title="${safeText(titulo)}">${safeText(rotulo)}</button>`;
}
function daFocusedDayDetailHtml(items,team) { if(!daControllerDayFocusIso) return ''; const info=daAgendaDayInfo(daControllerDayFocusIso); const dayItems=items.filter(item=>daControllerDate(item)===daControllerDayFocusIso && !['Para agendar','Agendado'].includes(operationalFlowStatus(item))).sort((a,b)=>daTacticalScore(a)-daTacticalScore(b)); const origemVaria=new Set(dayItems.map(i=>isRequestItem(i)?'s':'c')).size>1; const rows=dayItems.map(item=>{const owner=daTacticalOwner(item,team); const reference=`${daControllerDateMode==='veiculacao'?'Veiculação':'Prazo'} ${daControllerDateLabel(item)}`; return `<article class="da-day-focus-row"><span class="da-day-focus-avatar">${daTacticalOwnerEditor(item,owner,owner?.color || '#ff9d00')}</span><button type="button" class="da-day-focus-open" onclick="openItemWorkspace('${item.id}')" aria-label="Abrir contexto de ${safeText(item.nome)}"><span class="da-day-focus-copy"><b>${safeText(item.nome)}</b><small>${safeText(item.cliente||'Sem cliente')} · ${safeText(firstName(owner?.name||'Equipe'))}</small></span></button><span class="da-day-focus-status">${daTacticalFormatTag(item,origemVaria)}${daTacticalStatusTag(item,false,{semRiscoDaData:true})}</span><span class="da-day-focus-tools">${daBotaoDeDataHtml(item,reference)}</span></article>`;}).join('') || '<div class="da-empty-tactical">Nenhuma entrega criativa ativa neste dia.</div>'; return `<section class="da-day-focus-panel da-day-workspace"><div class="da-day-focus-head"><span><b>Fila do dia · ${info.name} ${info.date}</b> <span>· ${dayItems.length} entrega${dayItems.length===1?'':'s'} para conduzir</span></span></div><div class="da-day-focus-list">${rows}</div></section>`; }
function daCheckinDayBoardHtml(items,team){
  const today=HOJE_ISO||new Date().toISOString().slice(0,10); const iso=daControllerDayFocusIso||today; const info=daAgendaDayInfo(iso); const daily=items.filter(item=>daControllerDate(item)===iso&&!['Para agendar','Agendado'].includes(operationalFlowStatus(item))).sort((a,b)=>daTacticalScore(a)-daTacticalScore(b));
  if(!daily.length) return ''; const current=iso===today; const rows=daily.map(item=>{ const owner=daTacticalOwner(item,team); return `<article class="da-execution-row"><button type="button" class="da-execution-open" onclick="openItemWorkspace('${item.id}')" aria-label="Abrir contexto de ${safeText(item.nome)}"><span class="da-execution-owner">${daTacticalPersonVisual(owner,owner?.color||'#ff9d00')}</span><span class="da-execution-copy"><b>${safeText(item.nome)}</b><small>${safeText(item.cliente||'Sem cliente')} · ${safeText(firstName(owner?.name||'Equipe'))}</small></span><span class="da-execution-tags">${daTacticalFormatTag(item)}${daTacticalStatusTag(item,false,{semRiscoDaData:true})}</span></button>${daCheckinControlsHtml(item,owner)}</article>`; }).join('');
...
function daBarraHtml(range,selectedUser,selectedName) {
  ...
  const pessoa = selectedUser
    ...
    : `<span class="da-barra-quem vazia" title="Clique numa pessoa abaixo para ver só a carga dela"><b>Toda a célula</b></span>`;
...
// e risco alto é tudo que vence hoje: o dia de hoje inteiro virava alerta e a
// palavra deixava de separar alguma coisa. Agora só conta o que pede ação —
// peça atrasada que não foi finalizada, ou peça parada numa etapa de bloqueio.
// Uma peça atrasada e bloqueada conta uma vez, como bloqueada: é o que destrava.
function daAlertasDoDia(dayItems,iso,today) {
  let atrasadas=0, bloqueadas=0;
  dayItems.forEach(item=>{
    if (daControllerBlocked(item)) bloqueadas++;
    else if (iso<today && !isFinishedItem(item)) atrasadas++;
  });
  const partes=[];
  if (atrasadas) partes.push(`${atrasadas} atrasada${atrasadas===1?'':'s'}`);
  if (bloqueadas) partes.push(`${bloqueadas} bloqueada${bloqueadas===1?'':'s'}`);
  return {atrasadas, bloqueadas, total:atrasadas+bloqueadas, texto:partes.join(' · ')};
}
function daWeekAgendaHtml(items,range,team,today) {
  ...
  const rail=dates.map(iso=>{const info=daAgendaDayInfo(iso); const active=daControllerDayFocusIso===iso; const dayItems=items.filter(item=>daControllerDate(item)===iso && !['Para agendar','Agendado'].includes(operationalFlowStatus(item))); const alerta=daAlertasDoDia(dayItems,iso,today); const alerts=alerta.total; const owners=[...new Map(dayItems.map(item=>{const owner=daTacticalOwner(item,team); return [owner?.id||`none-${item.id}`,owner];})).values()].filter(Boolean).slice(0,4); const extraOwners=Math.max(0,new Set(dayItems.flatMap(item=>assignedIds(item))).size-owners.length); // Dia vazio dizia "Sem entrega" e, uma linha abaixo, "sem entrega". A segunda
    // linha existe para qualificar o que ha no dia; sem nada no dia ela nao tem o
    // que dizer, e some.
    const state=alerts?alerta.texto:(dayItems.length?'sem alerta imediato':''); return `<button type="button" class="da-week-rail-day ${active?'active':''} ${iso===today?'today':''} ${dayItems.length?'':'vazio'}" onclick="setDaControllerDayFocus('${iso}')" title="Abrir fila de ${info.name} ${info.date}"><span class="da-week-rail-top"><b>${info.name} <span>${info.date}</span></b></span><span class="da-week-rail-avatars">${owners.map(owner=>daTacticalPersonVisual(owner,owner.color)).join('')}${extraOwners?`<span class="da-tactical-person" title="Mais ${extraOwners} pessoa${extraOwners===1?'':'s'}"><span style="background:#5f4939">+${extraOwners}</span></span>`:''}</span><span class="da-week-rail-info"><b>${dayItems.length?`${dayItems.length} entrega${dayItems.length===1?'':'s'}`:'Sem entrega'}</b>${state?`<small class="${alerts?'risk':''}">${safeText(state)}</small>`:''}</span></button>`;}).join('');
  // Na semana, dia sem entrega ocupa meia coluna: sete colunas iguais davam o
  // mesmo espaço a um sábado vazio e a uma quinta com três peças. No mês não —
  // lá a grade é um calendário e as colunas precisam alinhar com os dias.
  const colunas = isMonth ? '' : ` style="grid-template-columns:${dates.map(iso=>items.some(item=>daControllerDate(item)===iso && !['Para agendar','Agendado'].includes(operationalFlowStatus(item)))?'minmax(118px,1fr)':'minmax(72px,.55fr)').join(' ')}"`;
  ...
  return `<section class="da-week-agenda">${cabeca}
    ...
    <div class="da-week-rail ${isMonth?'month':''}"${colunas}>${leading}${rail}</div></section>`;
}
...
// A linha da janela da Mesa de comando. Numa fila de "Publicar hoje" a data é a
// mesma em todas: ela só aparece quando varia (a fila de risco, por exemplo).
// O mesmo vale para "Conteúdo/Solicitação": só marca quando a fila mistura.
function daDailyCommandModalRow(item,{mostrarData=true,origemVaria=false}={}){ const owner=daTacticalOwner(item); const ref=daDailyCommandModalRef(item); const podeExcluir=typeof podeAdministrar==='function'&&podeAdministrar(); const apoio=[item.cliente||'Sem cliente', firstName(owner?.name||'Equipe'), mostrarData?ref:''].filter(Boolean).map(safeText).join(' · '); return `<div class="da-command-modal-row" role="button" tabindex="0" onclick="daFilaAbrirPeca('${item.id}',event)" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();daFilaAbrirPeca('${item.id}',event);}"><span class="da-command-modal-owner">${daTacticalPersonVisual(owner,owner?.color||'#ff9d00')}</span><span class="da-command-modal-copy"><b>${safeText(item.nome)}</b><small>${apoio}</small></span><span class="da-command-modal-tags">${daTacticalFormatTag(item,origemVaria)}${daTacticalStatusTag(item,true,{soAtraso:true})}</span>${podeExcluir?`<button type="button" class="da-fila-excluir" title="Excluir ${safeText(item.nome||'esta demanda')}" aria-label="Excluir demanda" onclick="event.stopPropagation();daFilaExcluirPeca('${item.id}')">${typeof ICONE!=='undefined'?ICONE.lixo:'×'}</button>`:''}</div>`; }
function daDailyCommandModalRef(item){ return item.veiculacao_iso?`Veiculação ${planningDateBr(item.veiculacao_iso).slice(0,5)}`:item.prazo_iso?`Prazo ${planningDateBr(item.prazo_iso).slice(0,5)}`:'Sem data'; }
function openDaDailyCommand(kind){ if(kind==='approval') return openDaApprovalRadar(); const spec=daDailyCommandSpec(kind); const opcoes={mostrarData:new Set(spec.items.map(daDailyCommandModalRef)).size>1, origemVaria:new Set(spec.items.map(i=>isRequestItem(i)?'s':'c')).size>1}; const rows=spec.items.length?spec.items.map(item=>daDailyCommandModalRow(item,opcoes)).join(''):'<div class="da-approval-radar-empty">Nenhuma demanda nesta fila.</div>'; const quando=!opcoes.mostrarData&&spec.items.length?` · ${safeText(daDailyCommandModalRef(spec.items[0]))}`:''; openWorkflowModal(`<div class="workflow-kicker"><span>${safeText(spec.kicker)}${quando}</span><button class="workflow-close" type="button" onclick="closeWorkflowModal()" aria-label="Fechar">×</button></div><h2 class="workflow-title">${spec.items.length} ${safeText(spec.label)}</h2><p class="workflow-copy">${safeText(spec.copy)}</p><div class="da-command-modal-list">${rows}</div>`); document.getElementById('workflow-modal')?.classList.add('da-command-modal'); }
function daDailyCommandHtml(){ const specs=['publish','produce','approval','risk'].map(daDailyCommandSpec); const cards=specs.map((spec,index)=>{ const vazia=spec.items.length===0; return `<button type="button" class="da-command-daily-card ${index===0&&!vazia?'is-primary':''} ${spec.kind==='risk'&&!vazia?'is-risk':''} ${vazia?'vazia':''}" style="--command-color:${spec.color}" ${vazia?'disabled aria-disabled="true"':`onclick="openDaDailyCommand('${spec.kind}')"`}><span class="da-command-daily-kicker">${safeText(spec.kicker)}</span><b class="da-command-daily-count">${spec.items.length}</b><span class="da-command-daily-label">${safeText(spec.label)}</span><span class="da-command-daily-copy">${safeText(spec.copy)}</span>${vazia?'':'<span class="da-command-daily-seta" aria-hidden="true">›</span>'}</button>`; }).join(''); return `<section class="da-command-daily"><div class="da-command-daily-head"><div><b>Mesa de comando · Hoje</b><small>Uma ordem única de decisão para a Direção de Arte.</small></div><span class="da-command-daily-rule">Prioridade: <strong>veicular</strong> → <strong>produzir</strong> → <strong>aprovar</strong> → <strong>destravar risco</strong>.</span></div><div class="da-command-daily-grid">${cards}</div></section>`; }
...
function daApprovalWaitText(item){ const clock=daStatusClock(item); const days=Math.max(0,Number(clock?.days||0)); if(!days) return 'Entrou hoje'; return `${days} ${days===1?'dia':'dias'} aguardando`; }
...
// Gráfico da mesa: semana ou mês, quantos períodos para trás/frente, e o dia
// clicado (que filtra a lista). O celular abre na semana: o mês não cabe legível.
const DA_PLANNING_GRAFICO = { periodo: (typeof matchMedia === 'function' && matchMedia('(max-width:760px)').matches) ? 'semana' : 'mes', deslocamento: 0, dia: '' };
...
// O que a lista mostra: o filtro da mesa e, se houver, o dia clicado no gráfico
// (peças com prazo OU veiculação naquele dia — as duas linhas do gráfico).
function daPlanningItensDaLista(userId){ const dia=DA_PLANNING_GRAFICO.dia; const itens=daIndividualPlanningItems(userId); return dia?itens.filter(item=>String(item.prazo_iso||'')===dia||String(item.veiculacao_iso||'')===dia):itens; }
...
function daPlanningToggleItem(userId,itemId,checked,event){
  const ordem=daPlanningItensDaLista(daPlanningPessoasDaMesa(userId)).map(item=>String(item.id));
...
// O GRÁFICO DA MESA — quantas peças caem em cada dia, por prazo e por veiculação.
//
// A faixa de números dizia o total; não dizia ONDE a carga estava. Duas linhas no
// mesmo eixo (as duas contam peças, a mesma unidade), a carga ideal tracejada e o
// hoje marcado. Cada dia é um botão: passar o mouse mostra os números, clicar
// filtra a lista para aquele dia. Os três números que pedem ação ficam como
// etiquetas, e só aparecem quando há o que fazer.
const DA_PLANNING_CORES = { prazo: '#d9702a', veic: '#4a86e8' };
function daPlanningDiasDoPeriodo() {
  const hoje = new Date(`${daPlanningTodayIso()}T12:00:00`);
  const iso = (d) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
  const g = DA_PLANNING_GRAFICO;
  if (g.periodo === 'semana') {
    const seg = new Date(hoje); seg.setDate(hoje.getDate() - ((hoje.getDay() + 6) % 7) + g.deslocamento * 7);
    return Array.from({ length: 7 }, (_, k) => { const d = new Date(seg); d.setDate(seg.getDate() + k); return iso(d); });
  }
  const inicio = new Date(hoje.getFullYear(), hoje.getMonth() + g.deslocamento, 1, 12);
  const total = new Date(inicio.getFullYear(), inicio.getMonth() + 1, 0, 12).getDate();
  return Array.from({ length: total }, (_, k) => iso(new Date(inicio.getFullYear(), inicio.getMonth(), k + 1, 12)));
}
function daPlanningRotuloDoPeriodo(dias) {
  const d = (iso) => new Date(`${iso}T12:00:00`);
  if (DA_PLANNING_GRAFICO.periodo === 'mes') {
    const m = d(dias[0]).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' });
    return m.charAt(0).toUpperCase() + m.slice(1);
  }
  return `${planningDateBr(dias[0]).slice(0, 5)} a ${planningDateBr(dias[dias.length - 1]).slice(0, 5)}`;
}
function daPlanningGraficoHtml(itens, contas, quem) {
  const hoje = daPlanningTodayIso();
  const g = DA_PLANNING_GRAFICO;
  const dias = daPlanningDiasDoPeriodo();
  const porDia = dias.map((dia) => ({
    dia,
    prazo: itens.filter((i) => String(i.prazo_iso || '') === dia).length,
    veic: itens.filter((i) => String(i.veiculacao_iso || '') === dia).length,
  }));
  const maior = Math.max(CARGA_IDEAL_POR_DIA + 1, ...porDia.map((p) => Math.max(p.prazo, p.veic)));
  const teto = Math.ceil(maior / 2) * 2;
  const W = 1000, H = 120, n = dias.length;
  const x = (k) => (n === 1 ? W / 2 : (k + 0.5) * (W / n));
  const y = (v) => H - (v / teto) * H;
  const linha = (campo) => porDia.map((p, k) => `${k ? 'L' : 'M'}${x(k).toFixed(1)} ${y(p[campo]).toFixed(1)}`).join(' ');
  const semana = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'];
  const nomeDia = (iso) => `${semana[new Date(`${iso}T12:00:00`).getDay()]} ${iso.slice(8, 10)}/${iso.slice(5, 7)}`;
  const mostraRotulo = (k, iso) => g.periodo === 'semana' || k === 0 || iso === hoje || (k + 1) % 5 === 0;
  ...
  const etiqueta = (valor, rotulo, tom, acao) => valor
    ? `<button type="button" class="mesa-etiqueta ${tom}" onclick="${acao}"><b>${valor}</b> ${rotulo}</button>` : '';
  const colunas = porDia.map((p, k) => {
    const fds = [0, 6].includes(new Date(`${p.dia}T12:00:00`).getDay());
    const resumo = `${nomeDia(p.dia)} · ${p.prazo} de prazo · ${p.veic} de veiculação`;
    return `<button type="button" class="mesa-grafico-dia${fds ? ' fds' : ''}${k >= n / 2 ? ' a-direita' : ''}${g.dia === p.dia ? ' escolhido' : ''}${p.dia === hoje ? ' hoje' : ''}"
      style="--x:${(x(k) / W * 100).toFixed(3)}%;--yp:${(y(p.prazo) / H * 100).toFixed(2)}%;--yv:${(y(p.veic) / H * 100).toFixed(2)}%"
      data-resumo="${safeText(resumo)}" onclick="daPlanningEscolherDia('${p.dia}')"
      aria-pressed="${g.dia === p.dia}" aria-label="${safeText(resumo)}. ${g.dia === p.dia ? 'Clique para ver todos os dias' : 'Clique para ver só este dia'}">
      <i class="p" aria-hidden="true"></i><i class="v" aria-hidden="true"></i></button>`;
  }).join('');
  const rotulos = porDia.map((p, k) => `<span class="${p.dia === hoje ? 'hoje' : ''}">${mostraRotulo(k, p.dia) ? (g.periodo === 'semana' ? nomeDia(p.dia) : p.dia.slice(8, 10)) : ''}</span>`).join('');
  const escolhido = g.dia ? `<button type="button" class="mesa-etiqueta escolhido" onclick="daPlanningEscolherDia('')" aria-label="Voltar a ver todos os dias">Só ${nomeDia(g.dia)} <span aria-hidden="true">×</span></button>` : '';
  return `<div class="mesa-grafico">
    <div class="mesa-grafico-topo">
      <div class="mesa-segmento" role="group" aria-label="Período do gráfico"><div>
        <button type="button" class="${g.periodo === 'semana' ? 'ativo' : ''}" aria-pressed="${g.periodo === 'semana'}" onclick="daPlanningPeriodoDoGrafico('semana')">Semana</button>
        <button type="button" class="${g.periodo === 'mes' ? 'ativo' : ''}" aria-pressed="${g.periodo === 'mes'}" onclick="daPlanningPeriodoDoGrafico('mes')">Mês</button></div></div>
      <div class="mesa-grafico-nav">
        <button type="button" onclick="daPlanningAndarNoGrafico(-1)" aria-label="${g.periodo === 'semana' ? 'Semana anterior' : 'Mês anterior'}">‹</button>
        <b>${safeText(daPlanningRotuloDoPeriodo(dias))}</b>
        <button type="button" onclick="daPlanningAndarNoGrafico(1)" aria-label="${g.periodo === 'semana' ? 'Próxima semana' : 'Próximo mês'}">›</button>
        ${g.deslocamento ? `<button type="button" class="mesa-grafico-hoje" onclick="daPlanningAndarNoGrafico(0)">Hoje</button>` : ''}
      </div>
      <div class="mesa-legenda" aria-hidden="true"><span><i style="background:${DA_PLANNING_CORES.prazo}"></i>Prazo</span><span><i style="background:${DA_PLANNING_CORES.veic}"></i>Veiculação</span><span><i class="tracejado"></i>Carga ideal · ${CARGA_IDEAL_POR_DIA}/dia</span></div>
      <div class="mesa-etiquetas">${escolhido}
        ${etiqueta(vencidos, vencidos === 1 ? 'prazo venceu' : 'prazos venceram', 'alerta', `daPlanningSetFilter('atrasados','${quem}')`)}
        ${etiqueta(contas.risk, 'abaixo do padrão', 'risco', `daPlanningSetFilter('fora7','${quem}')`)}
        ${etiqueta(semVeic, 'sem veiculação', 'risco', `daPlanningSetFilter('semveic','${quem}')`)}</div>
    </div>
    <div class="mesa-grafico-corpo">
      <span class="mesa-grafico-teto" aria-hidden="true">${teto}</span>
      <div class="mesa-grafico-area">
        <svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" aria-hidden="true">
          ${[0.5, 1].map((f) => `<line class="grade" x1="0" x2="${W}" y1="${(H - f * H).toFixed(1)}" y2="${(H - f * H).toFixed(1)}"/>`).join('')}
          <line class="base" x1="0" x2="${W}" y1="${H}" y2="${H}"/>
          <line class="carga" x1="0" x2="${W}" y1="${y(CARGA_IDEAL_POR_DIA).toFixed(1)}" y2="${y(CARGA_IDEAL_POR_DIA).toFixed(1)}"/>
          ${idxHoje >= 0 ? `<line class="hoje" x1="${x(idxHoje).toFixed(1)}" x2="${x(idxHoje).toFixed(1)}" y1="0" y2="${H}"/>` : ''}
          <path class="serie" d="${linha('veic')}" stroke="${DA_PLANNING_CORES.veic}"/>
          <path class="serie" d="${linha('prazo')}" stroke="${DA_PLANNING_CORES.prazo}"/>
        </svg>
        <div class="mesa-grafico-dias" role="group" aria-label="Demandas por dia: clique num dia para filtrar a lista">${colunas}</div>
      </div>
      <div class="mesa-grafico-rotulos" aria-hidden="true" style="--n:${n}">${rotulos}</div>
    </div>
  </div>`;
}
function daPlanningPeriodoDoGrafico(periodo) { DA_PLANNING_GRAFICO.periodo = periodo; DA_PLANNING_GRAFICO.deslocamento = 0; repintarMesaDePlanejamento(); }
function daPlanningAndarNoGrafico(passo) { DA_PLANNING_GRAFICO.deslocamento = passo ? DA_PLANNING_GRAFICO.deslocamento + passo : 0; repintarMesaDePlanejamento(); }
function daPlanningEscolherDia(dia) {
  DA_PLANNING_GRAFICO.dia = DA_PLANNING_GRAFICO.dia === dia ? '' : dia;
  // Marca de peça que sumiu da lista entraria num lote sem estar à vista.
  if (DA_PLANNING_GRAFICO.dia) SELECIONADAS.clear();
  repintarMesaDePlanejamento();
}
