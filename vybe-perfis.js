...
//
// Quem esta na mesa aparece aceso; quem divide a peca mas nao esta na mesa
// aparece apagado, porque tambem e responsavel e some-lo em silencio faria a
// peca parecer de uma pessoa so.
// Com mais de uma agenda na mesa, a lista se divide por pessoa — a mesma ideia
// da visao por grupos do GESTOR. Sem isso, somar agendas dava uma lista longa em
// que nao se sabia de quem era o que.
//
// A peca de dois donos NAO entra duas vezes. Ela vai para um bloco proprio no
// fim: duplicar traria de volta dois campos de prazo para a mesma data, e mudar
// um deixaria o outro desatualizado na frente de quem editou.
//
// A ordem escolhida dentro de cada bloco e a mesma da lista inteira, entao
// trocar a ordenacao continua valendo em todo lugar.
function daPlanningListaAgrupada(items, pessoas, userId) {
  const naMesa = pessoas.map(String);
  const equipe = typeof TEAM_USERS !== 'undefined' ? TEAM_USERS : [];
  const nomeDe = (id) => {
    const p = equipe.find((u) => String(u.id) === String(id));
    return p ? firstName(p.name) : 'Sem responsável';
  };
  const blocos = new Map(naMesa.map((id) => [id, []]));
  const compartilhadas = [];
  for (const item of items) {
    const donos = (typeof assignedIds === 'function' ? assignedIds(item) : []).map(String);
    const donosNaMesa = naMesa.filter((id) => donos.includes(id));
    if (donosNaMesa.length > 1) compartilhadas.push(item);
    else if (donosNaMesa.length === 1) blocos.get(donosNaMesa[0]).push(item);
    else compartilhadas.push(item);
  }
  let n = 0;
  const desenhar = (titulo, lista, extra = '') => {
    if (!lista.length) return '';
    const cabeca = `<div class="da-planning-grupo"><b>${safeText(titulo)}</b><small>${lista.length} ${lista.length === 1 ? 'entrega' : 'entregas'}</small>${extra}</div>`;
    return cabeca + lista.map((item) => daIndividualPlanningRow(item, n++, userId)).join('');
  };
  return naMesa.map((id) => desenhar(nomeDe(id), blocos.get(id) || [])).join('')
    + desenhar('Em mais de uma agenda', compartilhadas,
        '<span class="da-planning-grupo-nota">aparecem uma vez só</span>');
}

function daPlanningDonosHtml(item) {
  const ids = (typeof assignedIds === 'function' ? assignedIds(item) : []).map(String);
  if (!ids.length) return '<span class="da-planning-dono vazio">sem responsável</span>';
  const naMesa = new Set(daPlanningPessoasDaMesa().map(String));
  const equipe = typeof TEAM_USERS !== 'undefined' ? TEAM_USERS : [];
  return ids.map((id) => {
    const pessoa = equipe.find((u) => String(u.id) === id);
    const nome = pessoa ? firstName(pessoa.name) : 'Outro';
    return `<span class="da-planning-dono ${naMesa.has(id) ? 'na-mesa' : ''}"
      title="${safeText(pessoa?.name || 'Responsável')}">${safeText(nome)}</span>`;
  }).join('');
}
...
async function daPlanningCarregarArquivos() {
  ...
  const ids = celulas.map((c) => c.dataset.item).filter(Boolean);
  if (!ids.length) return;
  try {
    const r = await fetch(`/api/painel?area=arquivos&itens=${encodeURIComponent(ids.join(','))}`,
    ...
    celulas.forEach((c) => { if (c.isConnected) c.innerHTML = daPlanningArquivoHtml(c.dataset.item, mapa[c.dataset.item]); });
  } catch (erro) {
    ...
    // de sempre, e quem já tem arquivo descobre ao abrir a peça.
    celulas.forEach((c) => { if (c.isConnected) c.innerHTML = daPlanningArquivoVazioHtml(c.dataset.item); });
  }
}
...
async function daPlanningEnviarArquivo(input) {
  ...
  try {
    ...
    if (celula && celula.isConnected) {
      celula.innerHTML = daPlanningArquivoHtml(itemId, {
        total: 1, nome: file.name,
    ...
    showToast(`Não foi possível enviar: ${erro.message}`, 'err', 8000);
